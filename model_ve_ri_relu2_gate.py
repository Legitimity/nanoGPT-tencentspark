...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config, layer_idx):
        ...
        # gated attention: input-dependent sigmoid gate on the attention output
    ...
    def forward(self, x, value_embed=None, collect_head_stats=False):
        ...
        y = y * torch.sigmoid(self.gate(x)) # gated attention (Qiu et al. 2024 / speedrun)
...
class MLP(nn.Module):
    ...
    def forward(self, x):
        x = self.c_fc(x)
        x = F.relu(x) ** 2 # ReLU^2 activation (speedrun-validated, sparser than GELU)
        x = self.c_proj(x)
        x = self.dropout(x)
        return x
