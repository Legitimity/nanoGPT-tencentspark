...
class MLP(nn.Module):
    # Dense FFN for the non-MoE blocks in hybrid moe_layer_mode layouts. The
    # activation follows moe_activation so a hybrid run stays in one activation
    # family and the ablation isolates MoE placement only.

    def __init__(self, config):
        super().__init__()
        assert config.moe_activation in {'gelu', 'relu2'}, \
            "dense MLP supports only 'gelu'/'relu2'; use moe_layer_mode='all' for swiglu"
    ...
    def forward(self, x):
        x = self.c_fc(x)
        # square() stays bf16 under autocast (pow() would be promoted to fp32)
...
class MoE(nn.Module):
    ...
    def forward(self, x):
        ...
        # square() stays bf16 under autocast (pow() would be promoted to fp32,
        # tripling activation traffic); grouped_mm requires a uniform dtype
        ...
        # Switch-style load-balancing auxiliary loss (training only; only P carries
        # gradients). Returned functionally instead of stashed on the module:
        # attribute writes during forward cause dynamo graph breaks, which pushed
        # the cross-entropy at the end of GPT.forward out of the compiled region.
        aux = None
...
class GPTConfig:
    ...
    moe_expert_dim: int = 1536 # expert intermediate dim (= 2x n_embd; MoE layer is then exactly 2x the dense FFN params)
