...
class MLP(nn.Module):
    # Dense FFN for non-MoE blocks in hybrid layouts. Match the selected MoE
    # activation so single-GPU and DDP model semantics remain identical.
    ...
    def forward(self, x):
        x = self.c_fc(x)
        x = F.relu(x).square() if self.activation == 'relu2' else F.gelu(x)
        x = self.c_proj(x)
        x = self.dropout(x)
        return x, None # uniform (out, aux) interface with MoE


def _use_moe(config, layer_idx):
    """Decide whether a block uses the routed MoE FFN. Layer numbering in the
    mode names is 1-indexed: 'last6' = layers 7-12 (idx 6-11), 'even' = layers
    2,4,6,8,10,12 (idx 1,3,5,7,9,11)."""
    mode = config.moe_layer_mode
    if mode == 'all':
        return True
    if mode == 'last6':
        return layer_idx >= config.n_layer - 6
    if mode == 'even':
        return (layer_idx + 1) % 2 == 0
    raise ValueError(f"unknown moe_layer_mode: {mode!r}")


class MoE(nn.Module):
    """NVIDIA-style efficient LatentMoE with top-k unchanged.

    The router sees the original C-dimensional token. Only the dispatched
    payload and routed expert compute use latent width L. Shared down/up
    projections wrap the routed path; experts remain packed for grouped GEMM
    and per-expert Muon orthogonalization.
    """

    def __init__(self, config):
        super().__init__()
        C = config.n_embd
        L = config.moe_latent_dim
        ...
        assert config.moe_activation in {'gelu', 'relu2'}
        self.activation = config.moe_activation
        # Routing decisions use the original hidden representation, as in
        # LatentMoE; only the payload is compressed.
        self.router = nn.Linear(C, self.n_experts, bias=False)
        self.down_proj = nn.Linear(C, L, bias=False)
        self.up_proj = nn.Linear(L, C, bias=False)
        self.w1 = nn.Parameter(torch.empty(self.n_experts * L, D)) # [E*L, D]
        self.w2 = nn.Parameter(torch.empty(self.n_experts * D, L)) # [E*D, L]
        with torch.no_grad():
    ...
    def forward(self, x):
        ...
        NK = N * K
        xf = x.view(N, C)
        probs = F.softmax(self.router(xf), dim=-1) # router remains [C -> experts]
        top_p, top_i = probs.topk(K, dim=-1)
        if K > 1:
            top_p = top_p / top_p.sum(dim=-1, keepdim=True).clamp(min=1e-9)
        zf = self.down_proj(xf) # compressed routed payload [N, L]
        L = zf.size(-1)
        ...
        sorted_z = zf[flat_token[perm]]
        counts = torch.bincount(flat_expert, minlength=self.n_experts)
        offs = torch.cumsum(counts, dim=0).to(torch.int32)

        dt = torch.get_autocast_gpu_dtype() if (x.device.type == 'cuda' and torch.is_autocast_enabled('cuda')) else zf.dtype
        w1 = self.w1.view(self.n_experts, L, -1).to(dt)
        w2 = self.w2.view(self.n_experts, -1, L).to(dt)
        h = torch._grouped_mm(sorted_z.to(dt), w1, offs=offs)
        h = F.relu(h).square() if self.activation == 'relu2' else F.gelu(h)
        y = torch._grouped_mm(h.to(dt), w2, offs=offs) # latent expert outputs

        flat_weight = top_p.reshape(-1, 1)[perm].to(y.dtype)
        if K == 1:
            out_latent = torch.empty_like(zf)
            out_latent[flat_token[perm]] = (y * flat_weight).to(zf.dtype)
        else:
            y = (y * flat_weight).to(zf.dtype)
            out_latent = torch.zeros_like(zf)
            out_latent.index_add_(0, flat_token[perm], y)
        out = self.up_proj(out_latent).view(B, T, C)
...
class Block(nn.Module):
    ...
    def __init__(self, config, layer_idx):
        ...
        self.mlp = MoE(config) if _use_moe(config, layer_idx) else MLP(config)
...
class GPTConfig:
    ...
    moe_num_experts: int = 8 # alpha=2: double experts as routed width halves
    moe_top_k: int = 1 # efficient LatentMoE keeps baseline top-k unchanged
    moe_latent_dim: int = 384 # routed payload width, n_embd/2
    moe_expert_dim: int = 1536 # preserve nonlinear capacity per active expert
    ...
    moe_layer_mode: str = 'all' # which blocks use MoE FFN: 'all' | 'last6' (1-indexed layers 7-12) | 'even' (1-indexed layers 2,4,...,12); other blocks keep dense FFN
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        assert 0 < config.moe_latent_dim < config.n_embd
        assert config.moe_top_k <= config.moe_num_experts
        ...
        for pn, p in self.named_parameters():
            if pn.endswith('c_proj.weight') or pn.endswith('.mlp.up_proj.weight'):
                # LatentMoE up_proj is the residual-path output projection. Apply
                # tiny residual init here, not to every expert's latent w2.
    ...
    def forward(self, idx, targets=None, collect_head_stats=False):
        ...
        for layer_idx, block in enumerate(self.transformer.h):
            ...
            if collect_head_stats:
                x, head_stats, mlp_aux = block(
                    x, value_embed=layer_value_embed, collect_head_stats=True
                )
                all_head_stats.append(head_stats)
            else:
                x, mlp_aux = block(x, value_embed=layer_value_embed)
            if mlp_aux is not None:
                mlp_auxes.append(mlp_aux)
        ...
        if targets is not None:
            ...
            moe_aux = torch.stack(mlp_auxes).sum() if mlp_auxes else 0.0
    ...
    def from_pretrained(cls, model_type, override_args=None):
        ...
        allowed_overrides = {
            ...
            'resid_init_scale', 'moe_num_experts', 'moe_top_k',
            'moe_latent_dim', 'moe_expert_dim', 'moe_aux_loss_coef',
            'moe_activation', 'moe_layer_mode',
        }
        ...
        if config.moe_layer_mode != 'all':
            raise NotImplementedError(
                "from_pretrained only supports moe_layer_mode='all' (hybrid dense/MoE "
                "layouts are trained from scratch)"
            )
        model = GPT(config)
        sd = model.state_dict()
        sd_keys = sd.keys()
        ...
        newly_initialized = {
            ...
            or k.endswith('.mlp.up_proj.weight')
        }
    ...
    def estimate_mfu(self, fwdbwd_per_iter, dt):
        ...
        N -= (1 - self.config.moe_top_k / self.config.moe_num_experts) * sum(
