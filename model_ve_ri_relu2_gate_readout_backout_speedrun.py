...
Dense-FFN variant of the speedrun experiment: experiment 4 architecture
(Value Embeddings + resid_init_scale + gated attention + relu**2 dense FFN),
matching model_ve_ri_relu2_gate.py; the routed MoE FFN from the exp6/speedrun
model is removed entirely. The attention output gate stays fused into the QKV
projection (one [C, 4C] GEMM, identical math).
References:
