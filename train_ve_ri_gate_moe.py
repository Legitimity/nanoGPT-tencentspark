...
MoE-FFN variant of the speedrun experiment. Absorbs the current baseline's
speedrun mode (wall-clock limit, warmup rate estimate, collective stop),
fused-CE switch, embedding-decay fix and readout backout; the dense FFN is
replaced by the routed MoE (packed expert weights, Muon split per expert).
Muon momentum follows a linear warmup (0.85 -> 0.95 by default).
"""
...
from model_ve_ri_gate_moe import GPTConfig, GPT, Muon
...
moe_expert_dim = 1368 # expert intermediate dim per packed matrix (SwiGLU iso-param); must be a multiple of 8 for grouped GEMM
moe_aux_loss_coef = 0.01 # load-balancing loss weight (training only)
moe_activation = 'swiglu' # expert activation: 'swiglu' (default), 'relu2', or 'gelu'
# adamw optimizer
...
muon_momentum = 0.95 # Muon momentum (final value after the momentum warmup)
muon_momentum_warmup_init = 0.85 # momentum at update 0; linearly ramps to muon_momentum
muon_momentum_warmup_iters = 1000 # updates over which the momentum ramp completes; 0 disables the ramp
...
if init_from == 'scratch':
    ...
    checkpoint_model_keys = [
        ...
        'moe_num_experts', 'moe_top_k', 'moe_expert_dim',
        'moe_aux_loss_coef', 'moe_activation',
    ]
    missing_model_keys = [
...
if optimizer_name == 'muon':
    ...
    moe_packed_named = [(n, p) for n, p in muon_named if n.endswith(('.mlp.w1', '.mlp.w2', '.mlp.wg'))]
    regular_muon_named = [
        (n, p) for n, p in muon_named
        if not n.endswith('.attn.wqkv.weight') and not n.endswith(('.mlp.w1', '.mlp.w2', '.mlp.wg'))
    ]
    ...
    moe_matrices_per_layer = 3 if moe_activation == 'swiglu' else 2
    assert len(moe_packed_params) == moe_matrices_per_layer * model.config.n_layer, "expected packed expert matrices per MoE layer"
    ...
    print(
        ...
        f"{len(regular_muon_params) + len(qkv_muon_params) + len(moe_packed_params)} tensors, "
...
# Muon momentum warmup: linear ramp from muon_momentum_warmup_init to
# muon_momentum over muon_momentum_warmup_iters updates.
def get_muon_momentum(it):
    if muon_momentum_warmup_iters <= 0:
        return muon_momentum
    frac = min(it / muon_momentum_warmup_iters, 1.0)
    return muon_momentum_warmup_init + frac * (muon_momentum - muon_momentum_warmup_init)
...
while speedrun_mode or iter_num < target_num_updates:
    ...
    # Muon momentum warmup (0.85 -> 0.95 over muon_momentum_warmup_iters updates).
    scheduled_momentum = None
    if optimizer_name == 'muon':
        scheduled_momentum = get_muon_momentum(iter_num)
        for param_group in optimizers[0].param_groups:
            param_group['momentum'] = scheduled_momentum
    ...
    if (eval_only or iter_num % eval_interval == 0) and master_process:
        ...
        if wandb_log:
            ...
            if optimizer_name == 'muon':
                ...
                wandb_metrics["muon/momentum"] = scheduled_momentum
