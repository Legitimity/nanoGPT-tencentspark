...
Experiment 6 (speedrun variant): experiment 4 architecture (Value Embeddings +
resid_init_scale + gated attention + readout backout) with the dense FFN
replaced by experiment 5's routed MoE FFN (sort + torch._grouped_mm dispatch,
packed expert weights for Muon). Absorbs the current speedrun baseline features:
- training-path fused linear CE (cut-cross-entropy), eval keeps the plain path
- readout backout readout mixing ([1, 0] is baseline-equivalent at init)
- SwiGLU experts by default at iso-parameter dim 1368 (three packed matrices)
...
class MoE(nn.Module):
    ...
    def __init__(self, config):
        ...
        assert D % 8 == 0, "moe_expert_dim must be a multiple of 8 (torch._grouped_mm requires 16-byte-aligned strides in bf16)"
        self.n_experts = config.moe_num_experts
        self.top_k = config.moe_top_k
        self.aux_loss_coef = config.moe_aux_loss_coef
        assert config.moe_activation in {'gelu', 'relu2', 'swiglu'}
        self.activation = config.moe_activation
        self.router = nn.Linear(E, self.n_experts, bias=False)
        self.w1 = nn.Parameter(torch.empty(self.n_experts * E, D)) # [E*C, D] up-projection
        self.w2 = nn.Parameter(torch.empty(self.n_experts * D, E)) # [E*D, C] down-projection
        # SwiGLU adds a third packed matrix: the gate projection. D should then
        # be ~2/3 of the two-matrix expert_dim to keep total params constant.
        self.wg = nn.Parameter(torch.empty(self.n_experts * E, D)) if config.moe_activation == 'swiglu' else None # [E*C, D]
        with torch.no_grad():
    ...
    def forward(self, x):
        ...
        h = torch._grouped_mm(sorted_x.to(dt), w1, offs=offs) # [N*k, D] up-projection
        if self.activation == 'swiglu':
            wg = self.wg.view(self.n_experts, C, -1).to(dt)
            g = torch._grouped_mm(sorted_x.to(dt), wg, offs=offs) # [N*k, D] gate projection
            h = F.silu(g) * h # SwiGLU: silu(gate) * up
        elif self.activation == 'relu2':
            h = F.relu(h).square() # square() avoids autocast's fp32 promotion of pow()
        else:
            h = F.gelu(h)
        # grouped_mm is not autocast-aware and requires a uniform dtype, so cast
        # back explicitly (silu/gelu stay bf16; pow would have been promoted)
...
class GPTConfig:
    ...
    moe_expert_dim: int = 1368 # expert intermediate dim per packed matrix (SwiGLU iso-param: 3 matrices at 1368 ~= 2 at 2048; must be a multiple of 8 for grouped GEMM)
    moe_aux_loss_coef: float = 0.01 # Switch-style load-balancing loss weight (training only)
    moe_activation: str = 'swiglu' # expert activation: 'swiglu' (default; adds a third packed gate matrix), 'relu2', or 'gelu'
...
class GPT(nn.Module):
    ...
    def estimate_mfu(self, fwdbwd_per_iter, dt):
        ...
        N -= (1 - 1 / self.config.moe_num_experts) * sum(
            p.numel() for n, p in self.named_parameters() if n.endswith(('.mlp.w1', '.mlp.w2', '.mlp.wg'))
        )
        cfg = self.config
