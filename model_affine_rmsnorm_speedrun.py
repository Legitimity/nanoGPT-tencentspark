...
Affine-RMSNorm ablation of the current best dense-FFN speedrun architecture.
It preserves Value Embeddings, tiny residual initialization, gated attention,
ReLU² FFN and readout backout; only the two pre-norms per block and final norm
change from LayerNorm to affine RMSNorm.
References:
...
class RMSNorm(nn.Module):
    """Affine RMSNorm with optional shift for GPT-2 checkpoint compatibility."""
    ...
    def forward(self, input):
        output = F.rms_norm(input, self.weight.shape, self.weight, eps=1e-5)
