...
Dense-FFN SwiGLU variant of the speedrun experiment: experiment 4 architecture
(Value Embeddings + resid_init_scale + gated attention), with the relu**2 dense
FFN replaced by an iso-parameter dense SwiGLU FFN (silu(gate) * up, then down;
hidden dim 8/3 * n_embd = 2048, exactly matching the 4x MLP parameter count and
matmul FLOPs). The attention output gate stays fused into the QKV projection
(one [C, 4C] GEMM, identical math).
References:
...
class MLP(nn.Module):
    # Dense SwiGLU FFN, iso-parameter with the relu**2 4x MLP: three matrices at
    # hidden dim 8/3 * n_embd (= 2048 for 768) hold exactly the same parameter
    # count and matmul FLOPs as c_fc/c_proj at 4 * n_embd (= 3072).
    # The down projection keeps the c_proj name so the residual-projection
    # (ri-scaled) init below applies unchanged.

    def __init__(self, config):
        super().__init__()
        D = (4 * config.n_embd) * 2 // 3 # 2048 for n_embd=768 (iso-params)
        self.gate_proj = nn.Linear(config.n_embd, D, bias=config.bias)
        self.up_proj   = nn.Linear(config.n_embd, D, bias=config.bias)
        self.c_proj    = nn.Linear(D, config.n_embd, bias=config.bias)
        self.dropout   = nn.Dropout(config.dropout)

    def forward(self, x):
        # SwiGLU: silu(gate) * up, then down-projection (silu stays bf16 under autocast)
...
class GPT(nn.Module):
    ...
    def from_pretrained(cls, model_type, override_args=None):
        ...
        for k in sd_keys_hf:
            if '.mlp.' in k:
                # GPT-2's 3072-dim MLP does not match the iso-param SwiGLU dims
                # (2048); every FFN matrix stays newly initialized.
                continue
