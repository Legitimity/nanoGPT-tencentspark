...
from model_ve_ri_gate_latent_moe_speedrun import GPTConfig, GPT, Muon
...
# Efficient LatentMoE (alpha=2): router sees n_embd, routed payload is latent.
moe_num_experts = 8 # double expert count when routed width halves
moe_top_k = 1 # efficiency variant keeps top-k unchanged
moe_latent_dim = 384 # n_embd / 2
moe_expert_dim = 1536 # preserve expert nonlinear width
moe_aux_loss_coef = 0.01 # load-balancing loss weight (training only)
moe_activation = 'relu2' # expert activation; default relu**2 (exp2->exp3 was the biggest win); 'gelu' = exp5's MoE, run as exp7 for ablation
moe_layer_mode = 'all' # 'all' | 'last6' (layers 7-12) | 'even' (layers 2,4,...,12); other blocks keep dense FFN
# adamw optimizer
...
# fixed-wall-clock single-GPU mode. The timer starts immediately before the
...
if speedrun_mode:
    if ddp:
        raise ValueError("speedrun_mode is single-GPU only; launch with python3, not torchrun")
    ...
    print(
        f"speedrun mode: single GPU, {speedrun_time_limit_seconds:.1f}s training limit, "
        "no intermediate evaluation or periodic checkpoints; max_iters is ignored"
    )
if ddp:
...
model_args = dict(
    ...
    moe_latent_dim=moe_latent_dim, moe_expert_dim=moe_expert_dim,
    moe_aux_loss_coef=moe_aux_loss_coef,
    moe_layer_mode=moe_layer_mode,
...
if init_from == 'scratch':
    ...
    checkpoint_model_keys = [
        ...
        'resid_init_scale',
        'moe_num_experts', 'moe_top_k', 'moe_latent_dim', 'moe_expert_dim',
        'moe_aux_loss_coef', 'moe_activation', 'moe_layer_mode',
    ]
    missing_model_keys = [
    ...
    override_args = dict(
        ...
        moe_num_experts=moe_num_experts, moe_top_k=moe_top_k,
        moe_latent_dim=moe_latent_dim, moe_expert_dim=moe_expert_dim,
        moe_aux_loss_coef=moe_aux_loss_coef, moe_activation=moe_activation,
        moe_layer_mode=moe_layer_mode,
    )
...
if optimizer_name == 'muon':
    ...
    n_moe_layers = sum(
        1 for i in range(model.config.n_layer)
        if model.config.moe_layer_mode == 'all'
        or (model.config.moe_layer_mode == 'last6' and i >= model.config.n_layer - 6)
        or (model.config.moe_layer_mode == 'even' and (i + 1) % 2 == 0)
    )
    assert len(moe_packed_params) == 2 * n_moe_layers, \
        f"expected packed w1/w2 on {n_moe_layers} MoE layers, got {len(moe_packed_params)}"
    latent_proj_named = [
        (n, p) for n, p in regular_muon_named
        if n.endswith(('.mlp.down_proj.weight', '.mlp.up_proj.weight'))
    ]
    assert len(latent_proj_named) == 2 * n_moe_layers, \
        f"expected down/up projections on {n_moe_layers} LatentMoE layers"
    for n, p in moe_packed_named:
        expected_shape = (
            (model.config.moe_num_experts * model.config.moe_latent_dim, model.config.moe_expert_dim)
            if n.endswith('.mlp.w1') else
            (model.config.moe_num_experts * model.config.moe_expert_dim, model.config.moe_latent_dim)
        )
        assert p.shape == expected_shape, f"unexpected {n} shape: {tuple(p.shape)}"
    adam_decay = [
    ...
    logical_muon_matrices = (
        len(regular_muon_params) + 4 * len(qkv_muon_params)
        + model.config.moe_num_experts * len(moe_packed_params)
    )
    ...
    muon_groups = [
        ...
        {'params': moe_packed_params, 'muon_split_rows': model.config.moe_num_experts},
    ]
    optimizers = [
...
while speedrun_mode or iter_num < target_num_updates:
    ...
    # Speedrun mode has no intermediate evaluation or head-statistics forwards.
    if not speedrun_mode and (eval_only or iter_num % eval_interval == 0) and master_process:
    ...
    if speedrun_mode and speedrun_elapsed >= speedrun_time_limit_seconds:
        speedrun_termination_reason = 'time_limit'
        break
